...
func (r *ApplicationCredentialReconciler) reconcileNormal(
	...
	if instance.Status.SecretName != "" {
		...
		if err := r.Get(ctx, key, secret); err != nil {
			if k8s_errors.IsNotFound(err) {
				doRotate = true
				msg = "AC secret missing, rotating"
			} else {
				return ctrl.Result{}, err
			}
		}
	...
	if doRotate {
		...
		// Determine if this is a rotation (existing AC) vs initial creation
		...
		userID, err := r.getUserIDFromToken(userOS.GetOSClient(), instance.Spec.UserName)
		if err != nil {
			...
			instance.Status.Conditions.Set(condition.FalseCondition(
				...
				"Failed to get user ID from token: %s",
				err.Error(),
			))
			return ctrl.Result{}, err
		}
		...
		newACName := fmt.Sprintf("%s-%s", instance.Name, util.RandomString(5))
		...
		var previousExpiresMsg string
		if isRotation && instance.Status.ExpiresAt != nil {
			previousExpiresMsg = fmt.Sprintf("Previous AC expires: %s", instance.Status.ExpiresAt.Format(time.RFC3339))
		}
		...
		instance.Status.Conditions.MarkTrue(condition.ReadyCondition, "Application Credential is ready")
		...
		if isRotation {
			...
			r.EventRecorder.Event(
				...
				"CredentialRotated",
				fmt.Sprintf("ApplicationCredential rotated - EDPM nodes may need credential updates. %s", previousExpiresMsg),
			)
		...
		logger.Info("AC credential ready", "secret", secretName, "ACID", newID, "expiresAt", expiresAt, "nextCheck", nextCheck)
...
func (r *ApplicationCredentialReconciler) reconcileDelete(
	...
	// Before removing our CR finalizer, allow AC Secret to be deleted by removing its protection finalizer
	if instance.Status.SecretName != "" {
		key := types.NamespacedName{Namespace: instance.Namespace, Name: instance.Status.SecretName}
		secret := &corev1.Secret{}
		if err := r.Get(ctx, key, secret); err == nil {
...
func (r *ApplicationCredentialReconciler) getUserIDFromToken(identClient *gophercloud.ServiceClient, username string) (string, error) {
...
// It checks if the AC exists and if it's within the grace period before expiration.
